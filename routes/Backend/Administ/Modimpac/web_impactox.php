...
$routexxx = 'impactox';
$controll = 'Backend\Administ\Modimpac\ImpactoController@';
Route::group(['prefix' => 'impactos'], function () use ($routexxx, $controll) {
    Route::get('', [
        'uses' => $controll . 'index',
        'middleware' => ['permission:' . $routexxx . '-leerxxxx|' . $routexxx . '-crearxxx|' . $routexxx . '-editarxx|' . $routexxx . '-borrarxx']
    ])->name($routexxx);
    Route::get('listaxxx', [
        'uses' => $controll . 'getBImpactos',
        'middleware' => ['permission:' . $routexxx . '-leerxxxx|' . $routexxx . '-crearxxx|' . $routexxx . '-editarxx|' . $routexxx . '-borrarxx']
    ])->name($routexxx . '.listaxxx');
});

Route::group(['prefix' => 'impacto'], function () use ($routexxx, $controll) {
    Route::get('nuevo', [
