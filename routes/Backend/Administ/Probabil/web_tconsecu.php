...
$routexxx = 'tconsecu';
$controll = 'Backend\Administ\Probabil\TconsecuController@';
Route::group(['prefix' => 'tipo-consecuencias'], function () use ($routexxx, $controll) {
    Route::get('', [
        'uses' => $controll . 'index',
        'middleware' => ['permission:' . $routexxx . '-leerxxxx|' . $routexxx . '-crearxxx|' . $routexxx . '-editarxx|' . $routexxx . '-borrarxx']
    ])->name($routexxx);
    Route::get('listaxxx', [
        'uses' => $controll . 'getBTconsecus',
        'middleware' => ['permission:' . $routexxx . '-leerxxxx|' . $routexxx . '-crearxxx|' . $routexxx . '-editarxx|' . $routexxx . '-borrarxx']
    ])->name($routexxx . '.listaxxx');
});

Route::group(['prefix' => 'tipo-consecuencia'], function () use ($routexxx, $controll) {
    Route::get('nuevo', [
